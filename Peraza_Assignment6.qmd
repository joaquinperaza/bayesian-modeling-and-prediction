...
for(k in 2:K){
  ...
  mh <- ifelse(a1.try > 0 & a1.try < 1, dbeta(a1.try, alpha.a1, beta.a1) / dbeta(a1, alpha.a1, beta.a1),0)
  keep <- ifelse(mh>1,1,rbinom(1,1,mh))
  a1 <- ifelse(keep==1, a1.try, a1)
  
  #Metropolis-Hasting for [a2]
  a2.try <- rnorm(1,a2, sigma.tune)
  mh <- ifelse(a2.try > 0 & a2.try < 1, dbeta(a2.try, alpha.a2, beta.a2) / dbeta(a2, alpha.a2, beta.a2),0)
  keep <- ifelse(mh>1,1,rbinom(1,1,mh))
...
When we have priors for a1 (accuracy of rapid tests) and a2 (accuracy of PCR tests) in addition to p, it means that we have some prior beliefs or knowledge about the possible values of a1 and a2, for example, past studies, while still accounting for possibles uncertainties in this parameters, allowing more accurate estimations of \[p\] and \[z1\], thus, improve the probability of a correct numeric estimation of z1 (actual state of the tested bat).
